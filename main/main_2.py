...
load all details to postgres therefore cascading tables every time before loading
engine.execute("drop table if exists master_sp.dim_details_1 cascade;")

#three views are dependent on it so far..... we  gotta figure out a way to automatically create them everytime the program runs
df_merge_2.to_sql('dim_details_1', engine, schema = 'master_sp', if_exists='replace', index=False)
...
df_merge_2 = pd.read_sql("Drop table master_sp.dim_details_1 cascade", engine);

df_merge_2.to_sql("drop table parent", engine, if_exists="replace")
